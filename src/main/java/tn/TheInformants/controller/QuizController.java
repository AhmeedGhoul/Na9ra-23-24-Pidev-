...
private AnchorPane bgintt,highscore_int;
    private void displayQuestion() {
        ...
        if (index < questio.size()) { // Changed variable name to 'questio'
            ...
            Score scoree =new Score(score,quiz_id,1);
            try {
            ...
            for (Score qzz : s) {
                try {
                    FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("/gui/students/HighScoreItem.fxml"));
                    AnchorPane p = fxmlLoader.load();
        ...
        if (questions == null)
            ...
            try {
                ...
                for (int i = 0; i < results.size(); i++) {
                    ...
                    correctAnswer = StringEscapeUtils.unescapeHtml4(questionObj.get("correct_answer").getAsString());

                    // Create a new Question object and add it to the list
                    Question question = new Question();
                    question.setQuest(questionText);

                    // Set incorrect answers if available
                    if (incorrectAnswers.size() >= 1) {
                        question.setRep1(StringEscapeUtils.unescapeHtml4(incorrectAnswers.get(0).getAsString()));
                    }
                    if (incorrectAnswers.size() >= 2) {
                        question.setRep2(StringEscapeUtils.unescapeHtml4(incorrectAnswers.get(1).getAsString()));
                    }
                    if (incorrectAnswers.size() >= 3) {
                        question.setRep3(StringEscapeUtils.unescapeHtml4(incorrectAnswers.get(2).getAsString()));
                    }

                    // Set correct answer
                    question.setRep4(correctAnswer);
                    question.setRepc(correctAnswer);
                ...
                if (Objects.equals(correctAnswer, questions.get(currentIndex).getRep1())) {
                    t1.setSelected(true);
                    t2.setSelected(false);
                    t3.setSelected(false);
                    t4.setSelected(false);
                }
                if (Objects.equals(correctAnswer, questions.get(currentIndex).getRep2())) {
                    t2.setSelected(true);
                    t1.setSelected(false);
                    t3.setSelected(false);
                    t4.setSelected(false);
                }
                if (Objects.equals(correctAnswer, questions.get(currentIndex).getRep3())) {
                    t3.setSelected(true);
                    t2.setSelected(false);
                    t1.setSelected(false);
                    t4.setSelected(false);
                }
                if (Objects.equals(correctAnswer, questions.get(currentIndex).getRep4())) {
                    t4.setSelected(true);
                    t2.setSelected(false);
                    t3.setSelected(false);
                    t1.setSelected(false);
